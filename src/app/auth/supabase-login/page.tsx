...
export default function SupabaseLoginPage() {
  ...
  const handleEmailBlur = async (email: string) => {
    ...
    try {
      ...
      if (
        checkResult.exists &&
        !checkResult.availableProviders.includes("email") &&
        checkResult.availableProviders.length > 0
      ) {
        // OAuthのみで登録されている場合のみ警告
        setConflictInfo(checkResult);
        setError(
          generateAuthConflictMessage("login", checkResult.availableProviders)
        );
      } else {
        // メール/パスワード認証が可能、または未登録の場合は警告なし
    ...
    try {
      ...
      if (error) {
        if (error.message.includes("Email not confirmed")) {
          ...
          // 別のプロバイダーで登録されている可能性を再チェック
          try {
            const recheckResult = await checkEmailExists(email);
            if (
              recheckResult.exists &&
              !recheckResult.availableProviders.includes("email") &&
              recheckResult.availableProviders.length > 0
            ) {
              // OAuthのみで登録されている場合
  ...
  const handlePasswordReset = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const email = resetEmailValue.trim();
  ...
  if (showPasswordReset) {
    return (
      <AuthCard title="パスワードリセット">
        ...
        {error && (
          <div className="bg-destructive/10 text-destructive text-sm p-3 rounded-md border border-destructive/20 mb-6">
            <p>{error}</p>
          </div>
        )}
  ...
  return (
    <AuthCard title="Learning Journal にログイン">
      <div className="text-center mb-6">
        <p className="text-muted-foreground">
          アカウントにログインしてください
        </p>
                ...
                <div className="mt-3 pt-3 border-t border-destructive/20">
                  ...
                  <div className="bg-primary/10 border border-primary/20 rounded-md p-3">
                    ...
                    <ul className="text-sm text-muted-foreground space-y-1">
                      <li>• 上記の認証方法でログインしてください</li>
                      <li>
                        •
                        メール/パスワードでの登録を行う場合は新規登録してください
                      </li>
                    </ul>
                    <div className="mt-3">
                      <Link
                        href="/auth/supabase-register"
                        className="inline-flex items-center text-sm text-primary hover:text-primary/80 font-medium underline-offset-4 hover:underline"
                      >
                        <svg
      ...
      <form onSubmit={handleEmailLogin} className="space-y-6 mb-6">
        ...
        <Button
          ...
          disabled={
            loading !== null ||
            (conflictInfo?.exists &&
              !conflictInfo.availableProviders.includes("email") &&
              conflictInfo.availableProviders.length > 0)
          }
          className="w-full"
        >
